...
small_columns = data_cat.columns[data_cat.nunique() < 4]
big_columns = data_cat.columns[data_cat.nunique() >= 4]
...
encoder_small = ce.TargetEncoder()
data_cat_small = data_cat_small.apply(encoder_small.fit_transform, y)
valid_cat_small = valid_cat_small.apply(encoder_small.transform)

# Кодування для data_cat_big (OneHotEncoder)
encoder_big = OneHotEncoder(sparse=False)
data_cat_big = pd.DataFrame(
    encoder_big.fit_transform(data_cat_big),
    columns=encoder_big.get_feature_names_out(data_cat_train_big.columns)
)

valid_cat_big = pd.DataFrame(
    encoder_big.transform(valid_cat_big),
    columns=encoder_big.get_feature_names_out(valid_cat_big)
)
...
data_cat = pd.concat([data_cat_small, data_cat_big], axis=1)
...
# Вибір ознак на основі важливості
threshold = 0.001  # Ви можете вибрати інший поріг
