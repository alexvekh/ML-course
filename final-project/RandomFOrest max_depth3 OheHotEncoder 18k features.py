...
small_columns = data_cat_train.columns[data_cat_train.nunique() < 3]
big_columns = data_cat_train.columns[data_cat_train.nunique() >= 3]

# Розділення на data_cat_small та data_cat_big
data_cat_train_small = data_cat_train[small_columns]
data_cat_train_big = data_cat_train[big_columns]

data_cat_valid_small = data_cat_valid[small_columns]
data_cat_valid_big = data_cat_valid[big_columns]

# Кодування для data_cat_small (LabelEncoder)
encoder_small = LabelEncoder()
data_cat_train_small_encoded = data_cat_train_small.apply(encoder_small.fit_transform)
data_cat_valid_small_encoded = data_cat_valid_small.apply(encoder_small.transform)

# Кодування для data_cat_big (OneHotEncoder)
encoder_big = OneHotEncoder(sparse=False)
data_cat_train_big_encoded = pd.DataFrame(
    encoder_big.fit_transform(data_cat_train_big),
    columns=encoder_big.get_feature_names_out(data_cat_train_big.columns)
)

data_cat_valid_big_encoded = pd.DataFrame(
    encoder_big.transform(data_cat_valid_big),
    columns=encoder_big.get_feature_names_out(data_cat_big.columns)
)

# Вирівнювання індексів
data_cat_train_big_encoded.index = data_cat_train.index
data_cat_valid_big_encoded.index = data_cat_valid.index

# Об'єднання закодованих результатів
data_cat_train_encoded = pd.concat([data_cat_train_small_encoded, data_cat_big_encoded], axis=1)
data_cat_valid_encoded = pd.concat([data_cat_valid_small_encoded, data_cat_big_encoded], axis=1)

# Результат
print(data_cat_train_encoded.head())
print(data_cat_valid_encoded.head())


# %% # Розділення на data_cat_big та data_cat_small
data_cat_small = data_cat.loc[:, data_cat.nunique() < 3]
data_cat_big = data_cat.loc[:, data_cat.nunique() >= 3]

# Кодування для data_cat_small (LabelEncoder)
encoder_small = LabelEncoder()
data_cat_small_encoded = data_cat_small.apply(encoder_small.fit_transform)

# Кодування для data_cat_big (OneHotEncoder)
encoder_big = OneHotEncoder(sparse=False)
data_cat_big_encoded = pd.DataFrame(
    encoder_big.fit_transform(data_cat_big),
    columns=encoder_big.get_feature_names_out(data_cat_big.columns)
)

# Об'єднання результатів
data_cat_encoded = pd.concat([data_cat_small_encoded, data_cat_big_encoded], axis=1)
...
# Вибір ознак на основі важливості
threshold = 0.01  # Ви можете вибрати інший поріг
