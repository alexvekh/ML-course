...
data_number = num_imputer.fit_transform(data_number)
valid_number = num_imputer.transform(valid_number)  # Використовуємо transform замість fit
##%%
# data_numcat.fillna(0, inplace=True)
# valid_numcat.fillna(0, inplace=True)
numcat_imputer = SimpleImputer(strategy='median').set_output(transform='pandas')
data_numcat = num_imputer.fit_transform(data_numcat)
valid_numcat = num_imputer.transform(valid_numcat)  # Використовуємо transform замість fit
