...
# Розглянемо часовий ряд щоденних переглядів сторінки Вікіпедії, присвяченої Пейтону Меннінґу.
# Цей набір дозволить розглянути бібліотеки Prophet і statsmodels для роботи з рядом, який містить тренд, сезонність, викиди й пропущені значення, 
# а також врахувати у прогнозі особливі події (наприклад, такі як виступи Меннінга на іграх плей-офф та на суперкубку).

df = pd.read_csv('../datasets/mod_02_topic_04_ts_data.csv')
df.head()

# %%
# Бібліотека pandas містить широкий набір інструментів для маніпуляції з даними, представленими у форматі часових рядів.
# Для того щоб почати їх ефективно використовувати, необхідно правильно задати тип даних, який міститиме часову мітку.
# Зробимо це за допомогою методу pd.to_datetime(…). 
...
# %%
# Наступним кроком отримуємо описову статистику кількості відвідувачів сторінки.
df.describe()
...
# Завдяки логарифмуванню ми зможемо ефективно застосувати статистичні методи для видалення аномалій. Використаємо numpy.log(x).
df = np.log(df)0
df.head()

# %%
# При роботі з часовими рядами для тестового набору вибирається певна послідовність спостережень, зазвичай останніх у часовому ряді, а не випадкових спостережень, як у лінійній регресії.
